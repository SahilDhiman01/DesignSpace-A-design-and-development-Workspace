...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const erasePencilStroke = useMutation(
    ({ storage, setMyPresence }, point: Point) => {
      ...
      for (const layerId of liveLayerIds.toArray()) {
        ...
        if (layer && layer.get("type") === LayerType.Path) {
          const path = layer.get("points");
          const newPath = path.filter(
            ([x, y]) => Math.sqrt((x - point.x) ** 2 + (y - point.y) ** 2) > 10, // Only keep points outside the eraser radius
          );

          if (newPath.length !== path.length) {
            layer.update({ points: newPath });

            if (newPath.length === 0) {
              liveLayers.delete(layerId);
              liveLayerIds.delete(layerId);
            }
          }
        }
      }
  ...
  const onPointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (canvasState.mode === CanvasMode.Erasing) {
        erasePencilStroke(point); // Call the erase function on click
        return;
      }

      setCanvasState({ mode: CanvasMode.Pressing, origin: point });
    },
    [camera, canvasState.mode, setCanvasState, startDrawing, erasePencilStroke],
  );
